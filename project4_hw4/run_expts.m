...
%intial the median and mean values
expt_data_mean=zeros(length(Q_f),length(N),length(var));
expt_data_median=zeros(length(Q_f),length(N),length(var));

for ii = 1:length(Q_f)
    for jj = 1:length(N)
        for kk = 1:length(var)
            %expt_data_mat(ii,jj,kk) = median(computeOverfitMeasure(Q_f(ii),N(jj),1000,var(kk),500));
            expt_data_mean(ii,jj,kk) = mean(computeOverfitMeasure(Q_f(ii),N(jj),1000,var(kk),500));
            expt_data_median(ii,jj,kk) = median(computeOverfitMeasure(Q_f(ii),N(jj),1000,var(kk),500));
